    ...
    <div class="container-fluid p-0">
      ...
      <div class="container content">
        <div class="col-3">
          <div class="select">
            <div 
            v-for='(category,i) in leadersCategories' 
            :key="category.type" 
            class='w-100'>
            <button v-if="category.active==true" class="active w-100">{{category.label}}</button>
            <button v-on:click='ActivateCategory($event, i)' v-else class='w-100'>{{category.label}}</button>
            </div>
          </div>
        </div>
        <div class="col-9">
